...
interface AddMunicipalityModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (data: Municipality) => Promise<void>;
}

export default function AddMunicipalityModal({ isOpen, onClose, onAdd }: AddMunicipalityModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to create municipality');
    } finally {
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
          ...
          >
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-lg font-bold text-slate-900">Add New Municipality</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto custom-scrollbar space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" name="population" value={formData.population} onChange={handleChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-primary transition-all" />
                </div>
                <div className="space-y-1">
