...
export const read = async (req: Request, res: Response) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      res.status(404).json({ success: false, message: "Municipality not found" });
    }
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
...
export const add = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: "Municipality created", data: { id: docRef.id, ...newData } });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const edit = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: "Municipality updated", data: { id: req.params.id, ...updateData } });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
};
